...
input	int		InpTakeProfitPts		=  3000;			//	Take profit points
...
double myRsiArray[];
double myRsiDefinition = iRSI(Symbol(),Period(),RSI_Period,PRICE_CLOSE);
...
{
if (NormalizeDouble(myRsiArray[RsiLookBack],2) <= RSI_Buy_Value ) 
{
//Alert("Buy Order",askPrice);
PlaceOrderBuy(Sym,lotsize,askPrice,bidPrice,sl_value,tp_value);
}



else if ( NormalizeDouble(myRsiArray[RsiLookBack],2) >= 70 )
{
